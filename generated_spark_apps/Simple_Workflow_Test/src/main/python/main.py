"""
Main Application for Simple_Test
...
from workflows.wf_simple_test import Wfsimpletest
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Simple_Test Spark Application')
    ...
    try:
        logger.info("Starting Simple_Test Spark Application")
        
        # Load configuration
        config = load_config(args.config)
        
        # Create Spark session
        spark = create_spark_session("Simple_Test", config)
        
        # Create output directories
        os.makedirs("data/output", exist_ok=True)
        os.makedirs("logs", exist_ok=True)
        # Execute main workflow
        workflow = Wfsimpletest(spark, config)
        ...
        if workflow.validate_workflow():
            ...
            if success:
                logger.info("Simple_Test completed successfully")
                exit_code = 0
            else:
                logger.error("Simple_Test execution failed")
                exit_code = 1
        else:
