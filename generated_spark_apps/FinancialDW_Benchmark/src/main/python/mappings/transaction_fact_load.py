"""
Transaction_Fact_Load Mapping Implementation  
...
class TransactionFactLoad(BaseMapping):
    """Load transaction facts with multiple source integration"""
    
    def __init__(self, spark: SparkSession, config: Dict[str, Any]):
        super().__init__("Transaction_Fact_Load", spark, config)
    ...
    def execute(self) -> bool:
        """Execute the Transaction_Fact_Load mapping with production features"""
        start_time = datetime.now()
        
        try:
            self.logger.info("=" * 80)
            self.logger.info(f"Starting Transaction_Fact_Load mapping execution")
            self.logger.info("=" * 80)
            ...
            self.logger.info("Applying ACCOUNT_TRANSACTIONS_DB2 (advanced_aggregation) transformation...")
            ...
            self._update_metrics(f"account_transactions_db2_duration_seconds", transformation_time)
            self._update_metrics(f"account_transactions_db2_output_records", current_df.count())
            
            self.logger.info(f"ACCOUNT_TRANSACTIONS_DB2 completed in {transformation_time:.2f} seconds")
            self.logger.info("Applying ACCOUNT_MASTER (advanced_aggregation) transformation...")
            transformation_start = datetime.now()
            current_df = self.apply_advanced_aggregation_transformation(current_df)
            
            # Record transformation metrics
            transformation_time = (datetime.now() - transformation_start).total_seconds()
            self._update_metrics(f"account_master_duration_seconds", transformation_time)
            self._update_metrics(f"account_master_output_records", current_df.count())
            
            self.logger.info(f"ACCOUNT_MASTER completed in {transformation_time:.2f} seconds")
            self.logger.info("Applying JOIN_ACCOUNT_CUSTOMER (complex_lookup) transformation...")
            transformation_start = datetime.now()
            lookup_df = self._load_lookup_data_5()
            current_df = self.apply_complex_lookup_transformation(current_df, lookup_df)
            
            # Record transformation metrics
            transformation_time = (datetime.now() - transformation_start).total_seconds()
            self._update_metrics(f"join_account_customer_duration_seconds", transformation_time)
            self._update_metrics(f"join_account_customer_output_records", current_df.count())
            
            self.logger.info(f"JOIN_ACCOUNT_CUSTOMER completed in {transformation_time:.2f} seconds")
            self.logger.info("Applying ENRICH_WITH_CUSTOMER (complex_lookup) transformation...")
            transformation_start = datetime.now()
            lookup_df = self._load_lookup_data_6()
            current_df = self.apply_complex_lookup_transformation(current_df, lookup_df)
            
            # Record transformation metrics
            transformation_time = (datetime.now() - transformation_start).total_seconds()
            self._update_metrics(f"enrich_with_customer_duration_seconds", transformation_time)
            self._update_metrics(f"enrich_with_customer_output_records", current_df.count())
            
            self.logger.info(f"ENRICH_WITH_CUSTOMER completed in {transformation_time:.2f} seconds")
            self.logger.info("Applying ENRICH_WITH_PRODUCT (complex_lookup) transformation...")
            transformation_start = datetime.now()
            ...
            # Record transformation metrics
            transformation_time = (datetime.now() - transformation_start).total_seconds()
            self._update_metrics(f"enrich_with_product_duration_seconds", transformation_time)
            self._update_metrics(f"enrich_with_product_output_records", current_df.count())
            
            self.logger.info(f"ENRICH_WITH_PRODUCT completed in {transformation_time:.2f} seconds")
            ...
            self.logger.info("=" * 80)
            self.logger.info(f"Transaction_Fact_Load mapping completed successfully in {total_time:.2f} seconds")
            self.logger.info("=" * 80)
            ...
            self.logger.error(f"Transaction_Fact_Load mapping FAILED after {execution_time:.2f} seconds")
